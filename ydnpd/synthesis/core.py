...
try:
    ...
    warnings.warn("SynthCity is not installed. Please install it to use SynthCity synthesizers.")
...
def generate_synthetic_data(dataset: pd.DataFrame, schema: dict,
    ...
    # SDV
        ...
        synthesizer = getattr(sdv_single_table, f"{synth_name}Synthesizer")(metadata, **hparams)
    ...
    # SmartNoise
        ...
        synthesizer = snsynth_Synthesizer.create(synth_name, epsilon=epsilon, verbose=True, **hparams)
        synthesizer.fit(dataset, preprocessor_eps=preprocessor_eps,
